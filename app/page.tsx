...
const EXHIBITOR_KEYS: (keyof Exhibitor)[] = [
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        for (const item of parsed) {
          ...
          // Backward compat: map "secteur" to "categories"
...
function ToolCallIndicator({ toolName, state, input }: { toolName: string; state: string; input: unknown }) {
...
export default function Home() {
  ...
  const uniqueSectors = useMemo(() => {
    return new Set(allExhibitors.map((e) => e.categories).filter(Boolean)).size;
  }, [allExhibitors]);
